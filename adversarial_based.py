...
for epoch in range(opt.n_epochs):
    ...
    for i in range(120):    
        ...
        adv_loss = torch.nn.functional.kl_div(soft_S.log(), soft_T, reduction='batchmean')
        loss_gen = -adv_loss
        loss_gen.backward()
        optimizer_G.step()
        ...
        optimizer_S.step()
        if i == 1:
            print(f"[Epoch {epoch}/{opt.n_epochs}] [adv_loss: {adv_loss.item():.6f}] [kd_loss: {kd_loss.item():.6f}]")
